...
void loop() {
    ...
    bool canGoRight = (dist1 > WALL_THRESHOLD);
    bool canGoForward = (dist2 > WALL_THRESHOLD);
    bool canGoLeft = (dist3 > WALL_THRESHOLD);

    // Dead End Handling
    if (!canGoRight && !canGoForward && !canGoLeft) {
        if (s.empty()) {
            stopMovement();
            return;
        }
        ...
        } else if (backtrackAction.first == "re_trai") {
            turnRight(speed, TURN_ANGLE, start); // Turn Right
        } else if (backtrackAction.first == "thang") {
            di_thang(speed); // Move backwards
        }
        Serial.print("Backtracking: "); Serial.println(backtrackAction.first.c_str());
    } else {
        // Prioritize unexplored
        if (canGoForward) {
            s.push(std::make_pair("thang", 1));
            di_thang(speed);
            ...
            s.push(std::make_pair("re_trai", 1)); // Corrected push for backtracking
            start = getYaw();
            turnRight(speed, TURN_ANGLE, start);
