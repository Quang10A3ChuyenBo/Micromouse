...
// 1 ô di chuyển = 2450 xung (encoder tick per cell)
const float ENCODER_TICK_PER_REV = 2450.0;
const float WHEEL_BASE = 100.0;      // mm

float robotX = 0, robotY = 0;
// Hướng của xe được quản lý bằng biến currentDirection:
...
// ================ TCA Select Function ================
const uint8_t sensorChannels[3] = {1, 2, 4}; // Sensor 1: forward, 2: left, 3: right
...
// ================ Dummy resetSensors() ================
void resetSensors() {
  // Thêm code reset nếu cần; ở đây để trống.
}
...
// Mỗi 90° quay = 600 xung. Khi quay phải, bánh trái tăng, bánh phải giảm; khi quay trái, ngược lại.
void turnRightFixed(int spd, float turnFactor) {
  long startLeft = encoder1.getCount();
  long startRight = encoder2.getCount();
  float required = 600.0 * turnFactor;
  while (true) {
    long leftDiff = encoder1.getCount() - startLeft;    // Bánh trái tiến (tăng)
    long rightDiff = encoder2.getCount() - startRight;    // Bánh phải lùi (giá trị âm)
    if (leftDiff >= required && rightDiff <= -required) break;
...
void turnLeftFixed(int spd, float turnFactor) {
  ...
  float required = 600.0 * turnFactor;
  while (true) {
    long leftDiff = encoder1.getCount() - startLeft;    // Bánh trái lùi (âm)
    long rightDiff = encoder2.getCount() - startRight;    // Bánh phải tiến
    if (leftDiff <= -required && rightDiff >= required) break;
...
// ================ Update Position ================
// Khi di chuyển thẳng, ta chỉ tính khoảng cách trung bình và cập nhật theo currentDirection.
void updateRobotPosition() {
  ...
  float avgEnc = (dEnc1 + dEnc2) / 2.0;
  float distance = (avgEnc / ENCODER_TICK_PER_REV) * (PI * WHEEL_DIAMETER);
  if (currentDirection == 0) robotY += distance;
  else if (currentDirection == 1) robotX += distance;
  else if (currentDirection == 2) robotY -= distance;
  else if (currentDirection == 3) robotX -= distance;
  initialEnc1 = curEnc1;
  initialEnc2 = curEnc2;
}
...
void dfsDecision(int dist_forward, int dist_left, int dist_right) {
  ...
  // Tính ô phía trước dựa trên currentDirection
  int nx, ny;
  ...
  if (rightDir == 0) { rx = currentCellX; ry = currentCellY + 1; }
  else if (rightDir == 1) { rx = currentCellX + 1; ry = currentCellY; }
  else if (rightDir == 2) { rx = currentCellX; ry = currentCellY - 1; }
  ...
  if (leftDir == 0) { lx = currentCellX; ly = currentCellY + 1; }
  else if (leftDir == 1) { lx = currentCellX + 1; ly = currentCellY; }
  else if (leftDir == 2) { lx = currentCellX; ly = currentCellY - 1; }
...
void setup() {
  ...
  resetSensors(); // Hàm resetSensors() được định nghĩa dưới dạng rỗng
  renderMenu();
